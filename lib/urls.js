...
import { getErrorMessage, getInvalidExtensions } from "./utils.js";
 ...
 */
export async function getInternalLinksFromPlaywrightPage({ page, data, link }) {
  try {
    ...
    for (const siteLink of siteLinks) {
      try {
        ...
        logging(`${link} - Error processing link: ${getErrorMessage(error)}`);
      }
    }
  } catch (error) {
    logging(
      `${link} - Failed to get internal links: ${getErrorMessage(error)}`
    );
  }
  return;
}
...
export function getExternalUrlInfos({ data, url, headers, page }) {
  ...
  if (externalLinks.has(hostname)) {
    ...
    externalLinks.set(hostname, {
      ...
      firstAddedOnPageNumber: data.pagesVisited,
 ...
 * @param {string} params.link - The link to check.
 */
export async function getInternalLinksFromPagePuppeteer({ data, link }) {
...
export function addExtendedSitePaths(data) {
  if (!data.options.extendedSitePaths) return;
 ...
 */
export function addInternalLink({ link, data }) {
