...
export async function clickElement(page, features) {
  ...
  try {
    ...
    logging(
      `${page.url()} - Error clicking element: ${getErrorMessage(error)}`
    );
  }
  return;
}
...
export const scrollToPageEnd = async (page) => {
  ...
  do {
    try {
      ...
      logging(
        `${page.url()} - Error while scrolling: ${getErrorMessage(error)}`
      );
      break;
    }
...
/**
 * Simulates random mouse movements within the browser window.
 * @param {Page} page - The Playwright page instance.
 * @returns {Promise<void>} A promise that resolves when the simulation is complete.
 */
export async function simulateMouseMovementPlaywright(page) {
...
/**
 * Simulates random mouse movements within the browser window.
 * @param {PagePuppeteer} page - The Playwright page instance.
 * @returns {Promise<void>} A promise that resolves when the simulation is complete.
 */
export async function simulateMouseMovementPuppeteer(page) {
