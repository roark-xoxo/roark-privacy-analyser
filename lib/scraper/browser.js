/**
 * @typedef {import('./types').BrowserModes} BrowserModes
...
/**
 * Retrieves a Playwright browser instance with error handling.
 * @param {object} args - The mode for the browser (headful or headless).
 * @param {BrowserModes|undefined} args.browserMode - The mode for the browser (headful or headless).
 * @param {BrowserType} args.browserType - The type of browser.
 * @returns {Promise<{browser: Browser | null, message: string | null}>} An object containing the browser instance or an error message.
 */
export const getPlaywrightBrowser = async ({ browserMode, browserType }) => {
  try {
    return {
      browser: await getBrowserPlaywright({
        headless: browserMode === "headful" ? false : true,
        browserType,
      }),
      message: null,
    };
