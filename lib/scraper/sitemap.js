 ...
 * @param {ScraperArgsFeaturesType} features - The features for scraping.
 */
export async function getLinksFromSitemapUrl(
  page,
  url,
  links,
  origin,
  ...
  const log = features?.log ?? false;
  if (log) logToFile("Internal Links before Sitemap:", links.size);
  try {
    const sitemapUrl = new URL(url);
    await addLinksFromSitemap(page, sitemapUrl, links, origin);
  } catch (_err) {
    if (log) logToFile("fn_getLinksFromSitemapUrl", _err);
  }
  if (log) logToFile("Internal Links after Sitemap:", links.size);
  return;
}
...
async function addLinksFromSitemap(page, url, links, origin) {
  try {
    ...
    logToFile(`Error processing sitemap links: ${errorMessage}`);
  }
}
...
async function getLinksFromSitemapPage(page, href) {
  try {
    ...
    const errorMessage = getErrorMessage(error);
    logToFile(`Failed to get links from page: ${errorMessage}`);
    return [];
  }
}
 ...
 */
export async function searchSitemaps({ page, data, features }) {
  if (features?.sitemapSearch) {
    await getlinksFromSitemapSearch(page, data.url.origin, data.internalLinks);
  }

  if (features?.sitemapUrl)
