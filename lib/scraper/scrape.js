/**
 * @typedef {import('../scraper/types').AirtableUpdateScraperArgs} AirtableUpdateScraperArgs
 ...
 * @typedef {import('../scraper/types').ScraperArgsBody} ScraperArgsBody
 * @typedef {import('../scraper/types').ScraperArgsFeaturesType} ScraperArgsFeaturesType
...
export function getScrapeResults(args) {
  ...
  process.on("unhandledRejection", function () {
    if (features.log) {
      logToFile("Unhandled Rejection at:", url);
    }
  });

  return runScraper({ url, features, airtable });
}
 ...
 */
async function runScraper({ url, features, airtable }) {
  if (features.engine === "puppeteer") {
    return getScrapeResultsPuppeteer({ url, features, airtable });
  } else if (features.engine === "both") {
    return combineResults({
      resultsPuppeteer: await getScrapeResultsPuppeteer({
        url,
        features,
        airtable,
      }),
      resultsPlaywright: await getScrapeResultsPlaywright({
        url,
        features,
        airtable,
      }),
  ...
  return getScrapeResultsPlaywright({ url, features, airtable });
}
