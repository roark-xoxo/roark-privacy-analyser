 ...
 * @param {string} message - The parameters to log. Can be of any type.
 * @param {object} options - The parameters to log. Can be of any type.
 */
export function logging(message, options = {}) {
  ...
  try {
    if (logType === "CONSOLE") {
      ...
      process.send({ log: `${new Date()}: ${message}` });
    }
  } catch (error) {
    console.error(error);
  }
